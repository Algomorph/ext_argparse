...
provisions_duration_days = Parameters.lembras_bread.value * 2
```
...
`python3 -m estimate_hero_success.py --name="Samwise Gamgee" --lembras_bread=25 --height=1.21`
...
`lembras_bread = Parameter(arg_type=int, required=True, shorthand="lembr")`
...
`python3 -m estimate_hero_success.py --name="Gandalf the Grey" --lembras_bread=5 --height=1.82 --species=WIZARD`
...
`python3 -m estimate_hero_success.py --lembras_bread=25  --settings_file=lord_of_the_rings/the_two_towers/SamwiseConfig.yaml`

The command line above will read the parameter values in [YAML](https://yaml.org/) format from the specified file and 
_override_ the `lembras_bread` setting with the argument `25`. The setting file `SamwiseConfig.yaml` for the example 
...
`python3 -m estimate_hero_success.py --lembras_bread=25  --settings_file=lord_of_the_rings/the_two_towers/SamwiseConfig.yaml --save_settings`
...
provisions_duration_days = Parameters.hero.lembras_bread.value * 2
...
`python3 -m estimate_hero_success.py --hero.name="Saruman the White" --hero.lembras_bread=0 --hero.height=1.84 --quest.name="Corruption of Rohan" --quest.year=3014 --quest.destination=Edoras`
